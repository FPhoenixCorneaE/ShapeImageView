...
import com.fphoenixcorneae.shapeimageview.DisplayUtils
import com.fphoenixcorneae.shapeimageview.R
...
class CircleProgressView @JvmOverloads constructor(
    ...
    private var mReachBarSize = DisplayUtils.dip2px(getContext(), 2f) // 未完成进度条大小
    private var mNormalBarSize = DisplayUtils.dip2px(getContext(), 2f) // 未完成进度条大小
    private var mReachBarColor = Color.parseColor("#108ee9") // 已完成进度颜色
    private var mNormalBarColor = Color.parseColor("#FFD3D6DA") // 未完成进度颜色
    private var mTextSize = DisplayUtils.sp2px(getContext(), 14f) // 进度值字体大小
    ...
    private var mRadius = DisplayUtils.dip2px(getContext(), 20f) // 半径
    ...
    private var mInnerPadding = DisplayUtils.dip2px(getContext(), 1f) // 内部圆与外部圆间距
    ...
    private var rectInner // 内部圆环绘制区域
            : RectF? = null
    private var mOuterSize = DisplayUtils.dip2px(getContext(), 1f) // 外层圆环宽度
    ...
    private fun initPaint() {
        ...
        mNormalPaint!!.strokeWidth = mNormalBarSize
        mReachPaint = Paint()
        ...
        mReachPaint!!.strokeWidth = mReachBarSize
        ...
        if (mProgressStyle == ProgressStyle.FILL_IN_ARC) {
            ...
            mOutPaint!!.strokeWidth = mOuterSize
    ...
    private fun obtainAttributes(attrs: AttributeSet?) {
        ...
        mRadius =
            ta.getDimension(R.styleable.CircleProgressView_cpv_radius, mRadius)
        rectF =
            RectF((-mRadius), (-mRadius), mRadius, mRadius)
        when (mProgressStyle) {
            ProgressStyle.FILL_IN -> {
                mReachBarSize = 0f
                mNormalBarSize = 0f
    ...
    private fun drawNormalCircle(canvas: Canvas) {
        ...
        if (needDrawInnerBackground) {
            canvas.drawCircle(
                0f, 0f, mRadius - min(mReachBarSize, mNormalBarSize) / 2.toFloat(),
    ...
    var reachBarSize: Float
        get() = mReachBarSize
        set(reachBarSize) {
            mReachBarSize = DisplayUtils.dip2px(context, reachBarSize)
            invalidate()
        }

    var normalBarSize: Float
        get() = mNormalBarSize
        set(normalBarSize) {
            mNormalBarSize = DisplayUtils.dip2px(context, normalBarSize)
            invalidate()
        }
    ...
    var textSize: Int
        get() = mTextSize
        set(textSize) {
            mTextSize = DisplayUtils.sp2px(context, textSize.toFloat())
            invalidate()
        }
    ...
    var radius: Float
        get() = mRadius
        set(radius) {
            mRadius = DisplayUtils.dip2px(context, radius)
            invalidate()
        }
    ...
    var innerPadding: Float
        get() = mInnerPadding
        set(innerPadding) {
            mInnerPadding = DisplayUtils.dip2px(context, innerPadding)
            val mInnerRadius = mRadius - mOuterSize / 2 - mInnerPadding
            rectInner = RectF(
                -mInnerRadius,
                -mInnerRadius,
    ...
    var outerSize: Float
        get() = mOuterSize
        set(outerSize) {
            mOuterSize = DisplayUtils.dip2px(context, outerSize)
            invalidate()
        }
    ...
    override fun onSaveInstanceState(): Parcelable? {
        ...
        bundle.putFloat(INNER_PADDING, innerPadding)
        bundle.putInt(OUTER_COLOR, outerColor)
        bundle.putFloat(OUTER_SIZE, outerSize)
        // 保存text信息
        ...
        bundle.putFloat(REACH_BAR_SIZE, reachBarSize)

        // 保存未到达进度信息
        bundle.putInt(NORMAL_BAR_COLOR, normalBarColor)
        bundle.putFloat(NORMAL_BAR_SIZE, normalBarSize)
        return bundle
    }
    ...
    override fun onRestoreInstanceState(state: Parcelable) {
        if (state is Bundle) {
            ...
            mInnerPadding = state.getFloat(INNER_PADDING)
            mOuterColor = state.getInt(OUTER_COLOR)
            mOuterSize = state.getFloat(OUTER_SIZE)
            ...
            mReachBarSize = state.getFloat(REACH_BAR_SIZE)
            mNormalBarColor = state.getInt(NORMAL_BAR_COLOR)
            mNormalBarSize = state.getFloat(NORMAL_BAR_SIZE)
            initPaint()
